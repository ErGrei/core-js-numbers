...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const AngleBetweenVectors = Math.acos(a / b);
...
function roundToPowerOfTen(num, pow) {
  ...
  if (pow === 0) {
    ...
    ToPowerOfTen = Math.round(num / 10 ** pow) * 10 ** pow;
  }
  return ToPowerOfTen;
}
...
function isPowerOfTwo(num) {
  if (num === 0 || Math.log2(num) !== Math.floor(Math.log2(num))) {
    return false;
  }
  return true;
}
